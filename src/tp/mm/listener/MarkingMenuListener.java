...
public class MarkingMenuListener extends MouseInputAdapter {
	...
	private static double ANGLE_MIN = 0.44;// Entre 0 et 1 car valeur du cos de
											// l'angle
	...
	public void mousePressed(MouseEvent event) {
		...
		case IDLE:
			if (SwingUtilities.isRightMouseButton(event)) {
				...
				state = State.PRESS;
				taskAfficher = task.schedule(afficher, TIMER_AFF,
	...
	public void mouseDragged(MouseEvent event) {
		...
		case IDLE:
			break;
		case PRESS:
			futur = task.schedule(selection, TIMER_SELECT,
					TimeUnit.MILLISECONDS);
			buffer.add(current);
		...
		case DRAG:
			...
			if (origin.distance(current) > 45 && cosAngle < ANGLE_MIN
					&& cosAngle > (-ANGLE_MIN)) {
				origin = current;
